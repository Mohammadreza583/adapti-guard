"""Traceable experiment run artifacts — never stores API secrets."""
...
def redact_env() -> dict[str, Any]:
    return {
        ...
        "openai_api_key": "***REDACTED***" if os.getenv("OPENAI_API_KEY") else None,
        "key_present": bool(os.getenv("OPENROUTER_API_KEY") or os.getenv("OPENAI_API_KEY")),
    }
...
class ExperimentRunContext:
    ...
    def create(
        ...
        *,
        ...
        ctx = cls(experiment_id=experiment_id, run_id=run_id, run_dir=run_dir, config=config)
        ctx._open_logs()
    ...
    def write_initial_artifacts(self) -> None:
        ...
        (self.run_dir / "git_commit.txt").write_text(git_commit() or "UNKNOWN", encoding="utf-8")
        (self.run_dir / "command.txt").write_text(" ".join(sys.argv) + "\n", encoding="utf-8")
    ...
    def write_model_config(self, model_config: dict[str, Any]) -> None:
        ...
        for k in list(safe.keys()):
            if "api_key" in k.lower() or k.endswith("_key"):
                safe[k] = "***REDACTED***"
    ...
    def append_log(self, level: str, message: str, **fields: Any) -> None:
        ...
        entry = {"timestamp": utc_now_iso(), "level": level, "message": message, **fields}
        with path.open("a", encoding="utf-8") as handle:
...
def update_registry_row(registry_path: Path, row: dict[str, Any]) -> None:
    import csv

    registry_path.parent.mkdir(parents=True, exist_ok=True)
    fieldnames = [
        "experiment_id", "run_id", "timestamp", "status", "git_commit",
        "dataset", "dataset_hash", "target_model", "judge_model",
        "n_samples", "seed", "metrics_path", "run_dir", "notes",
    ]
